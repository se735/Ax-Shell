...
class MetricsProvider:
    ...
    def get_metrics(self):
        return (self.cpu, self.mem, self.temp, self.disk, self.gpu)
...
class Metrics(Box):
    def __init__(self, **kwargs):
        ...
        visible = getattr(data, "METRICS_VISIBLE", {'cpu': True, 'ram': True, 'temp': True, 'disk': True, 'gpu': True})
        ...
        self.temp = SingularMetric("temp", "TEMP", icons.temp) if visible.get('temp', True) else None
        self.disk = disks
        self.gpu = gpus
        ...
        if self.temp: self.temp.usage.set_sensitive(False)
    ...
    def update_status(self):
        cpu, mem, temp, disks, gpus = shared_provider.get_metrics()
        ...
        if self.temp:
            self.temp.usage.value = temp / 100.0
...
class MetricsSmall(Button):
    def __init__(self, **kwargs):
        ...
        visible = getattr(data, "METRICS_SMALL_VISIBLE", {'cpu': True, 'ram': True, 'temp': True, 'disk': True, 'gpu': True})
    ...
    def update_metrics(self):
        ...
        if self.temp:
            self.temp.circle.set_value(temp / 100.0)
            self.temp.level.set_label(f"{int(temp)}°")
        for i, disk in enumerate(self.disk):
            # Ensure index is within bounds for disks list
            if i < len(disks):
